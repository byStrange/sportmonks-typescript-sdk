...
export interface PollingOptions {
  ...
  compareFunction?: (oldData: any, newData: any) => boolean;
}
...
export class Poller<T> {
  ...
  private hasDataChanged(newData: T): boolean {
    ...
    if (this.isPaginatedResponse(newData) && this.isPaginatedResponse(this.lastData)) {
      // Compare by IDs
      const oldIds = new Set(this.lastData.data.map((item: any) => item.id));
      const newIds = new Set(newData.data.map((item: any) => item.id));
  ...
  private isPaginatedResponse(data: any): data is PaginatedResponse<any> {
    return data && Array.isArray(data.data) && 'pagination' in data;
  }
}
...
export function createLivescoresPoller(
  fetchFunction: () => Promise<PaginatedResponse<any>>,
  options: Partial<PollingOptions> = {}
): Poller<PaginatedResponse<any>> {
...
export function createTransfersPoller(
  ...
  const defaultOptions: PollingOptions = {
    ...
    compareFunction: (oldData, newData) => {
      ...
      if (!oldData.data.length || !newData.data.length) {
        return true;
      }

      const oldLatest = oldData.data
        .map((t: any) => new Date(t.date).getTime())
        .sort((a: number, b: number) => b - a)[0];

      const newLatest = newData.data
        .map((t: any) => new Date(t.date).getTime())
        .sort((a: number, b: number) => b - a)[0];
